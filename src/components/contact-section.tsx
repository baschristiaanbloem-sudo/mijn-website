    ...
    <section id="contact" className="py-16 md:py-20">
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="bk-section-label">{t.contact.label}</p>
          <h1 className="mt-2 font-heading text-2xl font-bold uppercase tracking-wide text-foreground md:text-3xl">
            {t.contact.title}
          </h1>
          <p className="mt-4 text-lg leading-relaxed text-pretty text-muted-foreground">{t.contact.intro}</p>
        </div>
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
            ...
            <ul className="mt-6 divide-y divide-border">
              {hourRows.map((item) => (
                <li key={item.key} className="flex items-center justify-between py-3">
                  <span className="font-medium text-foreground">{t.contact.days[item.key]}</span>
                  <span className={item.muted ? "text-muted-foreground" : "text-foreground"}>
                    {item.muted ? t.common.closed : item.hours}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="flex flex-col gap-5">
            <div className="bk-card p-7 md:p-8">
              ...
              <h2 className="mt-4 font-heading text-xl font-medium text-foreground">{t.contact.addressTitle}</h2>
              <p className="mt-2 leading-relaxed text-muted-foreground">
                <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer" className="font-medium text-primary hover:underline">
                  {practiceAddress}
                </a>
              </p>
            </div>
            ...
            <div className="bk-card border-primary/30 bg-accent p-7 md:p-8">
              ...
              <p className="mt-3">
                <a href={`mailto:${practiceEmail}`} className="font-medium text-primary hover:underline">
                  {practiceEmail}
                </a>
              </p>
            </div>
            ...
            <div className="bk-card p-7 md:p-8">
              ...
              <h2 className="mt-4 font-heading text-xl font-medium text-foreground">{t.contact.emergencyTitle}</h2>
              <p className="mt-2 leading-relaxed text-muted-foreground">{t.contact.emergencyText}</p>
            </div>
          </div>
        </div>
      </div>
